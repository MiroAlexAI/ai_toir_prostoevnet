...
export default function Home() {
  ...
  const handleEquipmentSubmit = async (data) => {
    ...
    try {
      const prompt = `Проанализируй вводные данные: 
Участок: ${data.site}
Тип оборудования: ${data.type}
Модель: ${data.model}
Год: ${data.year}
Производитель: ${data.manufacturer}

Задание:
1. Проверь валидность оборудования (status: valid/invalid). 
   КРИТЕРИЙ: Если 'Тип оборудования' - это реально существующий вид промышленной техники, то статус 'valid'. 
   ИГНОРИРУЙ поле 'Производитель' - оно может быть любым или пустым, это не повод для 'invalid'.
2. Если в поле 'Тип' написана абракадабра или не техника, укажи статус 'invalid', причину и предложи ОДИН реально существующий пример 'Типа оборудования'.
3. Найди 3 реальных ближайших аналога этого оборудования.

Ответь СТРОГО В ФОРМАТЕ JSON следующей структуры:
{
  "status": "valid" или "invalid",
  "reason": "причина если invalid",
  "suggestion": "пример типа оборудования для проверки если invalid",
